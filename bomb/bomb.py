...
msg = ">>---> Pressione ctrl + c para parar o bot."
...
def clickGreenBarButtons():
    ...
    if len(not_working_green_bars) >=2:
        clickBtn(images['all'], name ='all', timeout=2)
        clickBtn(images['x'])
        return
...
def goToHeroes():
    ...
    if clickBtn(images['hero-icon'], name ='hero', timeout=20):
        login_attempts = 0
...
def login(username, password):
    ...
    if clickBtn(images['username'], name='sign button', timeout= 2):
...
def refreshHeroes():
    ...
    if clickBtn(images['treasure-hunt-icon'], name='treasure'):
        logger('Todos os Heróis foram enviados para o mapa')
        login_attempts = 0
    else:
    ...
    while True:
        ...
        while tl <=7:
            ...
            for wx in windows:
                ...
                # username = loginList[loginXT][0]
                # password = loginList[loginXT][1]

                # if clickBtn(images['connect-wallet']) or clickBtn(images['ok'], name='okBtn'):
                #     sys.stdout.flush()
                #     last["login"] = now
                #     login(username, password)
